...
import { StakingContract, DalTokenContract, ScanTokenContract } from "../../abi";
...
export const loadAppDetails = createAsyncThunk(
    ...
    async ({ networkID, provider }: ILoadAppDetails) => {
        ...
        const totalSupply = (await scanContract.totalSupply()) / Math.pow(10, 9);
        const circSupply = (await dalContract.circulatingSupply()) / Math.pow(10, 9);
        ...
        const timeBondsAmountsPromises = allBonds.map(bond => bond.getScanAmount(networkID, provider));
        ...
        const circ = await dalContract.circulatingSupply();
