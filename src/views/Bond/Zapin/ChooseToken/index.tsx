...
import { busd, wbnb } from "../../../../helpers/bond";
import { busd as busdToken, wbnb as wbnbToken } from "../../../../helpers/tokens";
...
function ChooseToken({ open, handleClose, handleSelect, bond }: IChooseTokenProps) {
    ...
    const filtredTokens = tokens.filter(({ name, address, isBnb }) => {
        ...
        if (bond.name === busd.name) {
            lpFilter = busdToken.address !== address;
        }

        if (bond.name === wbnb.name) {
            lpFilter = isBnb ? false : wbnbToken.address !== address;
        }
