...
export class LPBond extends Bond {
    ...
    public getScanAmount(networkID: Networks, provider: StaticJsonRpcProvider) {
    ...
    private async getReserves(networkID: Networks, provider: StaticJsonRpcProvider, isToken: boolean): Promise<number> {
        ...
        const isScan = token1.toLowerCase() === addresses.SCAN_ADDRESS.toLowerCase();

        return isToken ? this.toTokenDecimal(false, isScan ? reserve0 : reserve1) : this.toTokenDecimal(true, isScan ? reserve1 : reserve0);
    }

    private toTokenDecimal(isScan: boolean, reserve: number) {
        return isScan ? reserve / Math.pow(10, 9) : reserve / Math.pow(10, 18);
    }
}
